...
export async function handleRule010(
  req: RuleRequest,
  determineOutcome: (
    value: number,
  ...
  const evaluationIntervalTime =
    ruleConfig.config.parameters.evaluationIntervalTime;
  ...
  const results = await databaseManager._pseudonymsDb.query(
    ...
    [debtorAccountId, 'pacs.002.001.12', 'ACCC', currentPacs002TimeFrame],
  );
