...
export const handleRule026 = async (
  ...
  const maxQueryRange: number = Number(
    ruleConfig.config.parameters.maxQueryRange,
  );
  ...
  const res = await databaseManager._pseudonymsDb.query(
   ...
   allSuccessfulPacs002 AS (
       ...
       AND txsts = 'ACCC'
       and (extract(epoch from (select credttm::timestamptz from newestPacs008) - credttm::timestamptz) * 1000) <= $3
       and (extract(epoch from (select credttm::timestamptz from newestPacs008) - credttm::timestamptz) * 1000) > 0
  ...
  const recentSuccessfulTransactionsAndTargetAmount = res.rows as Array<{
    targetAmount: number;
    historicalAmounts: number[];
  }>;
